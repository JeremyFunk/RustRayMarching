...
impl MaterialRaw{
    pub fn new(albedo: f64!(), specular: f64!(), diffuse: f64!(), n_specular: f64!()) -> MaterialRaw{
        MaterialRaw{
...
impl Material{
    pub fn new(albedo: f64!(), specular: f64!(), diffuse: f64!(), n_specular: f64!()) -> Material{
        Material { albedo, specular, diffuse, n_specular }
    }
    
    pub fn new_base() -> Material{
        Material { albedo: f64!(0.18), specular: f64!(0.2), diffuse: f64!(0.8), n_specular: f64!(10.0) }
    }
    
    pub fn new_direct() -> Material{
        Material { albedo: f64!(1.0), specular: f64!(0.0), diffuse: f64!(1.0), n_specular: f64!(0.0) }
    }
}
...
impl GroupPrimitive{
    pub fn new_union(prims: Vec<Box<dyn Primitive>>) -> GroupPrimitive {
        GroupPrimitive{
            ...
            prims: prims,
            primtive_data: get_primitive_data(f64v!([0.0, 0.0, 0.0]), f64v!([0.0, 0.0, 0.0]), f64v!([1.0, 1.0, 1.0]), Vec::new()),
            factor_f: f64!(0.0),
            factor: 0.0
        }
    }
    ...
    pub fn new_subtraction_smooth(prims: Vec<Box<dyn Primitive>>, factor: f64!()) -> GroupPrimitive {
        let f = get_f64!(factor);
        GroupPrimitive{
    ...
    pub fn new_intersection_smooth(prims: Vec<Box<dyn Primitive>>, factor: f64!()) -> GroupPrimitive {
        let f = get_f64!(factor);
        GroupPrimitive{
        ...
        match self.group_primitive_type {
            ...
            GroupModifierType::SmoothUnion => {
                for p in 1..self.prims.len() {
                    ...
                    distance.distance = helpers::mix(cur_dist.distance, distance.distance, h) - self.factor * h * (1.0 - h);
                }
            }
...
impl Sphere{
    pub fn new(rad: f64!(), pos: [f64!();3], rot: [f64!();3], scale: [f64!();3], pos_modifier: Vec<Box<dyn modifier::PosModifier>>, material: Option<Material>) -> Sphere {
        let m = material.unwrap_or(Material::new_direct());
        Sphere{
            rad,
            primtive_data: get_primitive_data(pos, rot, scale, pos_modifier),
            material_raw: MaterialRaw::new(m.albedo.clone(), m.specular.clone(), m.diffuse.clone(), m.n_specular.clone()),
            material: m,
        }
    }
}
...
impl InternalPrimitive for Sphere{
    fn _map_primitive(&self, pos: [f64;3]) -> PrimitiveResult{
        ...
        PrimitiveResult{distance, fractal_data: [0.0, 0.0, 0.0], material: self.material_raw}
    }
    fn get_primitive_data(&self) -> &PrimitiveData{
        return &self.primtive_data;
    }
...
impl Torus{
    pub fn new(rad: f64!(), ring_rad: f64!(), pos: [f64!();3], rot: [f64!();3], scale: [f64!();3], pos_modifier: Vec<Box<dyn modifier::PosModifier>>, material: Option<Material>) -> Torus {
        let m = material.unwrap_or(Material::new_direct());
        Torus{
            ring_rad,
            rad,
...
impl Cube{
    pub fn new(bounds: [f64!();3], pos: [f64!();3], rot: [f64!();3], scale: [f64!();3], pos_modifier: Vec<Box<dyn modifier::PosModifier>>, material: Option<Material>) -> Cube {
        let m = material.unwrap_or(Material::new_direct());
        Cube{
            bounds,
            primtive_data: get_primitive_data(pos, rot, scale, pos_modifier),
...
impl InternalPrimitive for Mandelbulb{
    fn _map_primitive(&self, pos: [f64;3]) -> PrimitiveResult{
        ...
        PrimitiveResult{distance, fractal_data: [iterations as f64, iterations as f64, iterations as f64], material: self.material_raw}
    }
    fn get_primitive_data(&self) -> &PrimitiveData{
        return &self.primtive_data;
    }
